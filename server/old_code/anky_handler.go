...
// 	var session types.WritingSession
...
// 	user, ok := privyUser.(types.PrivyUser)
// 	if !ok {
...
// func processWithLLM(session types.WritingSession) (*types.Anky, error) {
...
// 	uploadResult, err := uploadImageToCloudinary(imageHandler, chosenImageURL, session.ID.String())
// 	if err != nil {
